...
右键托盘图标 → 「设置」，在浏览器中打开 **Web 设置界面**，包含 7 个功能标签页：
...
| **游戏管理** | 查看当前运行中的游戏，手动添加或移除游戏目录 |
| **忽略名单** | 添加不想被统计的程序，支持从当前运行进程中选择添加、移除、清空全部 |
| **数据库管理** | 查看数据库大小与记录数、预览最近数据、清理旧数据、备份数据库 |
| **反馈与日志** | 调整日志等级（DEBUG/INFO/WARNING/ERROR）、查看运行日志、提交反馈（自动打开反馈文件夹） |

> **v0.3.0 说明**：设置界面已从 tkinter GUI 全面迁移至 **Web UI**（通过 `http://127.0.0.1:19234` 访问）。系统托盘图标保留，用于快速查看报告和退出程序。
...
| 文件 / 文件夹 | 说明 |
|:---|:---|
| `src/` | 核心业务模块（追踪引擎、数据存储、报告生成、通知系统、国际化、Bridge HTTP 服务） |
| `ui/` | Web 设置界面（HTML/CSS/JS） |
...
|:---|:---|
| 语言 | Python 3.11+ |
| 界面 | Web UI（HTML/CSS/JS）+ pystray（系统托盘）+ Pillow |
| 数据库 | SQLite（标准库） |
| 图表 | Chart.js（内嵌至报告） |
| 国际化 | 自研轻量 JSON 翻译模块 |
...
- [x] **v0.2.0-beta** — 精美模式设置界面、事件驱动追踪（因 GUI 问题发布为 beta）
- [x] **v0.3.0** — 设置全面迁移至 Web UI；弃用 tkinter GUI；轮询模式（稳定）
