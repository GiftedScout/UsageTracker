...
def render_onboarding_page(theme: str = 'fairy') -> str:
    ...
    bg_style = (
        'background: linear-gradient(135deg, #FFF8E7 0%, #F0F7FF 50%, #F5F0FF 100%); color: #2C3E50;'
        if theme == 'fairy'
        else 'background: linear-gradient(135deg, #0a0f1a 0%, #141b2d 100%); color: #E0E6ED;'
    )
    accent = '#5B8C5A' if theme == 'fairy' else '#1E90FF'
    card_bg = 'rgba(255,255,255,0.85)' if theme == 'fairy' else 'rgba(26,35,50,0.85)'
    btn_hover = '#4A7A4A' if theme == 'fairy' else '#1AB2FF'
...
<style>
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .step {{ text-align: left; margin: 16px 0; padding: 14px 16px; border-radius: 10px;
                 background: {'rgba(91,140,90,0.08)' if theme=='fairy' else 'rgba(30,144,255,0.08)'}; }}
...
<div class="card">
    ...
    <p class="subtitle">v0.3.0 &nbsp;·&nbsp; 网页端设置已就绪</p>
    ...
    <button class="btn-primary" onclick="finishOnboarding()">完成，进入设置 →</button>
