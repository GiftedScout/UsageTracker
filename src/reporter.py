...
class HTMLReportGenerator:
    ...
    def generate_daily_report(self, report: DailyReport, theme: str = 'fairy_tale') -> str:
        ...
        if report_date == yesterday:
            header_title = f'{header_icon} 昨日时光记录 {header_icon}'
        else:
            header_title = f'{header_icon} {report_date.month}月{report_date.day}日 使用时长报告 {header_icon}'

        def pct(s, t):
            return f'{s/t*100:.1f}%' if t else '0%'

        cat_names = {'browser': '浏览器', 'game': '游戏', 'other': '其他应用'}
        cat_html = ''
        ...
        app_rows = ''
        # 构建 app → category 映射，用于进度条颜色
        app_to_cat: dict[str, str] = {}
        app_exe_map: dict[str, str] = {}  # app_name → exe_path
        if self._data_store:
        ...
        for app, secs in sorted(report.app_breakdown.items(), key=lambda x: x[1], reverse=True):
            ...
            app_rows += f"""<tr><td>{app}</td><td class="time-num">{_fmt_h_full(secs)}</td><td><div class="prog-wrap"><div class="prog-fill" style="width:{pct(secs,report.total_usage_seconds)};background:{cat_colors.get(cat_key, '#9e8070')}"></div></div></td><td class="time-num">{pct(secs, report.total_usage_seconds)}</td><td class="ignore-td"><button class="ignore-btn" onclick="ignoreApp('{safe_app}','{safe_exe}',this)" title="忽略此应用">✕</button></td></tr>"""
        ...
        cat_bar_labels = json.dumps(['浏览器', '游戏', '其他'], ensure_ascii=False)
        cat_bar_datasets = json.dumps([
            {'label': '浏览器', 'data': [bh, 0, 0], 'backgroundColor': cat_colors['browser'] + 'd1', 'borderRadius': 5},
            {'label': '游戏', 'data': [0, gh, 0], 'backgroundColor': cat_colors['game'] + 'd1', 'borderRadius': 5},
            {'label': '其他', 'data': [0, 0, oh], 'backgroundColor': cat_colors['other'] + 'bf', 'borderRadius': 5},
        ], ensure_ascii=False)
        ...
        js_chart_font = json.dumps(chart_font, ensure_ascii=False)  # JS 字面字符串，可在 {{ }} 内使用

        html = f"""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>使用时长报告 · {report.date}</title>
{chartjs_tag}
...
<div class="summary">
<div class="card total"><div class="lbl">总使用时长</div><div class="val">{_fmt_h_full(report.total_usage_seconds)}</div></div>
<div class="card browser"><div class="lbl">浏览器</div><div class="val">{_fmt_h_full(report.browser_seconds)}</div></div>
<div class="card game"><div class="lbl">游戏</div><div class="val">{_fmt_h_full(report.game_seconds)}</div></div>
<div class="card alert"><div class="lbl">提醒</div><div class="val">{report.alerts_triggered}次</div></div>
</div>
<div class="section"><h2>分类分布</h2><div class="chart-wrap"><canvas id="catChart"></canvas></div></div>
<div class="section"><h2>分类详情</h2><div class="cat-list">{cat_html}</div></div>
{('<div class="section"><h2>游戏分项</h2><div class="game-chart-wrap"><canvas id="gameChart"></canvas></div><div class="game-list" style="margin-top:12px">' + game_items_html + '</div></div>') if game_apps else ''}
<div class="section"><h2>应用明细</h2><table><thead><tr><th>应用</th><th>时长</th><th>占比</th><th>百分比</th><th>操作</th></tr></thead><tbody>{app_rows}</tbody></table></div>
<div class="footer">由 UsageTracker 生成</div>
</div><script>
new Chart(document.getElementById('catChart'), {{
    ...
    options:{{ responsive:true, maintainAspectRatio:false, indexAxis:'y',
        ...
        scales:{{ x:{{ beginAtZero:true, title:{{ display:true, text:'时长(h)', font:{{ family:{js_chart_font}, size:12 }} }},
...
function ignoreApp(app,exe,btn){{
  ...
  .then(r=>r.json()).then(d=>{{
    ...
    else{{ btn.disabled=false; btn.textContent='✕'; btn.style.opacity='1'; alert(d.msg||'操作失败'); }}
  }}).catch(()=>{{ btn.disabled=false; btn.textContent='✕'; btn.style.opacity='1'; alert('无法连接到 UsageTracker，请确保程序正在运行'); }});
}}
</script></body></html>"""
    ...
    def generate_weekly_report(self, week_start: str, week_end: str,
        ...
        for i, d in enumerate(day_data):
            t = d['game'] + d['browser'] + d['other']
            day_rows += f"""<tr><td>{wd_names[i]}</td><td>{d['date']}</td><td style="color:{cat_colors['game']}">{_fmt_h(d['game'])}</td><td style="color:{cat_colors['browser']}">{_fmt_h(d['browser'])}</td><td style="color:{cat_colors['other']}">{_fmt_h(d['other'])}</td><td><strong>{_fmt_h(t)}</strong></td></tr>"""

        js_labels = json.dumps(labels, ensure_ascii=False)
        js_datasets = json.dumps([
            {'label': '游戏', 'data': game_h, 'backgroundColor': cat_colors['game'] + 'd1'},
            {'label': '浏览器', 'data': browser_h, 'backgroundColor': cat_colors['browser'] + 'd1'},
            {'label': '其他', 'data': other_h, 'backgroundColor': cat_colors['other'] + 'c2'},
        ], ensure_ascii=False)
        tooltip_cb = "ctx => ' ' + ctx.dataset.label + ': ' + ctx.raw + 'h'"

        html = f"""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>周报 · {week_start} ~ {week_end}</title>
{chartjs_tag}
<style>{css}</style><style>.chart-wrap {{ height:300px; }}</style></head>
<body><div class="container">
<div class="header"><h1>{header_icon} 上周时光回顾 {header_icon}</h1><div class="subtitle">{week_start} ～ {week_end}</div></div>
<div class="summary">
<div class="card total"><div class="lbl">周合计</div><div class="val">{_fmt_h(total_all)}</div></div>
<div class="card game"><div class="lbl">游戏</div><div class="val">{_fmt_h(total_game)}</div></div>
<div class="card browser"><div class="lbl">浏览器</div><div class="val">{_fmt_h(total_browser)}</div></div>
<div class="card other"><div class="lbl">其他</div><div class="val">{_fmt_h(total_other)}</div></div>
</div>
<div class="section"><h2>每日使用分布</h2><div class="chart-wrap"><canvas id="weekChart"></canvas></div></div>
<div class="section"><h2>每日明细</h2><table><thead><tr><th>星期</th><th>日期</th><th>游戏</th><th>浏览器</th><th>其他</th><th>合计</th></tr></thead><tbody>{day_rows}</tbody></table></div>
<div class="footer">由 UsageTracker 生成</div>
</div><script>
new Chart(document.getElementById('weekChart'), {{
    ...
    options:{{ responsive:true, maintainAspectRatio:false,
        ...
        scales:{{ x:{{ stacked:true, title:{{ display:true, text:'日期', font:{{ family:{js_chart_font}, size:12 }} }},
            ticks:{{ font:{{ family:{js_chart_font} }} }} }},
            y:{{ stacked:true, beginAtZero:true, title:{{ display:true, text:'时长(h)', font:{{ family:{js_chart_font}, size:12 }} }},
            ticks:{{ font:{{ family:{js_chart_font} }}, callback: v => v+'h' }} }}
        }}
    }}
}});
    ...
    def generate_monthly_report(self, year: int, month: int,
        ...
        js_wd_datasets = json.dumps([
            {'label': '游戏', 'data': wd_game_h, 'backgroundColor': cat_colors['game'] + 'cc'},
            {'label': '浏览器', 'data': wd_browser_h, 'backgroundColor': cat_colors['browser'] + 'cc'},
            {'label': '其他', 'data': wd_other_h, 'backgroundColor': cat_colors['other'] + 'b3'},
        ], ensure_ascii=False)
        ...
        for wd in range(7):
            ...
            wd_occ_datasets.append(json.dumps([
                {'label': '游戏', 'data': [round(it['game']/3600, 2) for it in items], 'backgroundColor': cat_colors['game'] + 'cc'},
                {'label': '浏览器', 'data': [round(it['browser']/3600, 2) for it in items], 'backgroundColor': cat_colors['browser'] + 'cc'},
                {'label': '其他', 'data': [round(it['other']/3600, 2) for it in items], 'backgroundColor': cat_colors['other'] + 'b3'},
            ], ensure_ascii=False))
        ...
        week_datasets = json.dumps([
            {'label': '游戏', 'data': [round(w['game']/3600, 2) for w in week_bars], 'backgroundColor': cat_colors['game'] + 'cc'},
            {'label': '浏览器', 'data': [round(w['browser']/3600, 2) for w in week_bars], 'backgroundColor': cat_colors['browser'] + 'cc'},
            {'label': '其他', 'data': [round(w['other']/3600, 2) for w in week_bars], 'backgroundColor': cat_colors['other'] + 'b3'},
        ], ensure_ascii=False)
        ...
        for wd in range(7):
            ...
            if t <= 0:
                continue
            wd_cards += f"""<div class="wd-card" style="border-left-color:{wd_colors_arr[wd]}"><div class="wd-name">{wd_names[wd]}</div><div class="wd-total">{_fmt_h(t)}</div><div class="wd-sub"><span style="color:{cat_colors['game']}">游{_fmt_h(g)}</span> · <span style="color:{cat_colors['browser']}">浏{_fmt_h(b)}</span> · <span style="color:{cat_colors['other']}">其{_fmt_h(o)}</span></div></div>"""

        week_table_rows = ''
        for w in week_bars:
            t = w['game'] + w['browser'] + w['other']
            week_table_rows += f"""<tr><td>{w['label']}</td><td style="color:{cat_colors['game']}">{_fmt_h(w['game'])}</td><td style="color:{cat_colors['browser']}">{_fmt_h(w['browser'])}</td><td style="color:{cat_colors['other']}">{_fmt_h(w['other'])}</td><td><strong>{_fmt_h(t)}</strong></td></tr>"""
    ...
    options:{{ responsive:true, maintainAspectRatio:false,
        ...
        week_mini_html = ''.join(
            ...
            f'<div class="week-mini-sub"><span style="color:{cat_colors["game"]}">游{_fmt_h(w["game"])}</span> · '
            f'<span style="color:{cat_colors["browser"]}">浏{_fmt_h(w["browser"])}</span> · '
            f'<span style="color:{cat_colors["other"]}">其{_fmt_h(w["other"])}</span></div></div>'
            for w in week_bars)
...
<div class="header"><h1>{header_icon} {month_str} 时光画卷 {header_icon}</h1><div class="subtitle">UsageTracker 月度回顾</div></div>
<div class="summary">
<div class="card total"><div class="lbl">月合计</div><div class="val">{_fmt_h(total_all_m)}</div></div>
<div class="card game"><div class="lbl">游戏</div><div class="val">{_fmt_h(total_game_m)}</div></div>
<div class="card browser"><div class="lbl">浏览器</div><div class="val">{_fmt_h(total_browser_m)}</div></div>
<div class="card other"><div class="lbl">其他</div><div class="val">{_fmt_h(total_other_m)}</div></div>
</div>
<div class="section"><h2>按星期汇总</h2><div class="wd-grid">{wd_cards}</div><div class="chart-wrap"><canvas id="weekdaySummaryChart"></canvas></div></div>
<div class="section"><h2>各次星期分布</h2><div class="wd-occ-grid">{wd_occ_subcharts}</div></div>
<div class="section"><h2>各周总量对比</h2><div class="week-grid">{week_mini_html}</div><div class="chart-wrap" style="margin-top:14px"><canvas id="weekChart"></canvas></div>
<table><thead><tr><th>周次</th><th>游戏</th><th>浏览器</th><th>其他</th><th>合计</th></tr></thead><tbody>{week_table_rows}</tbody></table></div>
<div class="footer">由 UsageTracker 生成</div>
</div><script>
new Chart(document.getElementById('weekdaySummaryChart'), {{
    ...
    options:{{ responsive:true, maintainAspectRatio:false,
        plugins:{{ legend:{legend_opts}, tooltip:{{ mode:'index', intersect:false, callbacks:{{ label:{tooltip_cb} }} }} }},
        scales:{{ x:{{ stacked:true, title:{{ display:true, text:'星期', font:{{ family:{js_chart_font}, size:12 }} }},
            ticks:{{ font:{{ family:{js_chart_font} }} }} }},
            y:{{ stacked:true, beginAtZero:true,
                title:{{ display:true, text:'时长(h)', font:{{ family:{js_chart_font}, size:12 }} }},
                ticks:{{ font:{{ family:{js_chart_font} }}, callback: v => v+'h' }} }}
        }}
    }}
}});
...
new Chart(document.getElementById('weekChart'), {{
    ...
    options:{{ responsive:true, maintainAspectRatio:false,
        plugins:{{ legend:{legend_opts}, tooltip:{{ mode:'index', intersect:false, callbacks:{{ label:{tooltip_cb} }} }} }},
        scales:{{ x:{{ stacked:true, title:{{ display:true, text:'周次', font:{{ family:{js_chart_font}, size:12 }} }},
            ticks:{{ font:{{ family:{js_chart_font} }} }} }},
            y:{{ stacked:true, beginAtZero:true,
                title:{{ display:true, text:'时长(h)', font:{{ family:{js_chart_font}, size:12 }} }},
                ticks:{{ font:{{ family:{js_chart_font} }}, callback: v => v+'h' }} }}
